...
public class AssetServiceImpl implements AssetService {
    ...
    @Override
    public AssetResponseDto getCoupleAssetsWithoutLoan(Long userId) {
        ...
        List<AssetDto> assetStocks = assetRepository.findAllByCouple_CoupleIdAndIsHiddenFalseAndAccountTypeStock(
                        couple.getCoupleId())
                .stream()
                .map(this::convertToDto)
                .collect(Collectors.toList());

        Long sum = assetRepository.sumAmountByCouple_CoupleIdAndIsHiddenFalse(
                couple.getCoupleId());

        Long loanSum = assetRepository.sumAmountByCouple_CoupleIdAndIsHiddenFalseAndAccountTypeLoan(
                couple.getCoupleId());

        sum = (sum == null ? 0L : sum) - (loanSum == null ? 0L : loanSum);
        return new AssetResponseDto(sum);
    }
    ...
    public boolean insertNewLoan(AssetLoanDto assetLoanDto, Long userId) {
        ...
        assetLoan.setInitialAmount(assetLoanDto.getCurrentAmount());
    ...
    private AccountDto sendPostAboutInquireDepositProducts(String userKey, String accountNo, Long dailyTransferLimit,
